...
export default function CommentCell({
  ...
  if (hasContent) {
    return (
      ...
      >
        <p className={`text-[13px] text-navy whitespace-pre-wrap break-words ${!readOnly ? 'pr-7' : ''}`}>{initialValue}</p>
        {!readOnly && (
          <button
            onClick={e => { e.stopPropagation(); enterEdit() }}
            className="absolute top-2.5 right-2 p-1 rounded text-text-muted opacity-0 group-hover:opacity-100 hover:text-navy hover:bg-[#EBEBEB] transition-all"
            title="Edit comment"
          >
            <Pencil size={12} />
          </button>
